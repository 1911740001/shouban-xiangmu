$("#entershen .box .u2 .l3 .enterDdeng").click(function(){
    ...
    http.open("get",`http://192.168.0.105:8080/denglu?name=${$("#entershen .box .u2 .l1 .enteryonghu").val()}&mima=${$("#entershen .box .u2 .l2 .entermima").val()}`);
    http.send()
    http.onreadystatechange=function(){
        if(http.readyState===4){
            if(http.responseText==="success"){
                ...
                location.href="http://192.168.0.105/项目周/shouye.html"
            }
            else{
                alert("用户名或密码错误")
            }
